...
- Modify: `python/sglang/srt/peft/oft/layers.py:221-225`
...
In `python/sglang/srt/peft/oft/layers.py`, change only the QKV fused condition from:
...
Expected: only `python/sglang/srt/peft/oft/layers.py` and
...
```bash
git add \
  python/sglang/srt/peft/oft/layers.py \
  test/srt/oft/test_split_dense_merged_projection_dispatch.py
