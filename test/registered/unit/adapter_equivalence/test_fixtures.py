...
def _complete_dense_shapes() -> dict[str, tuple[int, int]]:
    return {
        **_dense_shapes(),
...
def _dense_lora_shapes() -> dict[str, tuple[int, int]]:
    return {
        name: shape
        for name, shape in _dense_shapes().items()
        if not name.endswith("k_proj")
    }
...
def _moe_shapes() -> dict[str, tuple[int, int]]:
    return {
        ...
        "model.layers.0.self_attn.k_proj": (64, 64),
        "model.layers.0.self_attn.v_proj": (64, 64),
...
def test_shared_oft_fixture_matches_native_non_split_checkpoint_layout(
    tmp_path, architecture
):
    shapes = (
        _complete_dense_shapes()
        if architecture == "dense"
        else {**_moe_shapes(), "model.embed_tokens": (128, 64), "lm_head": (64, 128)}
    )
    # Different output widths are valid: the shared rotation acts on inputs.
    shapes["model.layers.0.self_attn.k_proj"] = (
        shapes["model.layers.0.self_attn.q_proj"][0],
        32,
    )
    ...
    tensors = load_file(str(fixture.path / "adapter_model.safetensors"))
    prefix = "base_model.model."
    compact = 8128 if architecture == "dense" else 496
    expected = {
        "model.layers.0.self_attn.qkv_proj": (
            1 if architecture == "dense" else 2,
            compact,
        ),
        ...
        "model.embed_tokens": (1 if architecture == "dense" else 2, compact),
        "lm_head": (1 if architecture == "dense" else 2, compact),
    }
    if architecture == "dense":
        expected.update(
            {
                "model.layers.0.mlp.gate_up_proj": (1, compact),
                "model.layers.0.mlp.down_proj": (1, compact),
            }
        )
    else:
        expected.update(
            {
                "model.layers.0.mlp.experts.0.gate_proj": (1, compact),
                "model.layers.0.mlp.experts.0.down_proj": (2, compact),
            }
        )
    assert {name: tensor.shape for name, tensor in tensors.items()} == {
        f"{prefix}{name}.oft_R": shape for name, shape in expected.items()
    }
    config = json.loads((fixture.path / "adapter_config.json").read_text())
    assert config["target_modules"] == sorted(
        {name.rsplit(".", 1)[-1] for name in expected}
    )
    assert all(np.any(tensor != 0) for tensor in tensors.values())


@pytest.mark.parametrize(
    "architecture,sibling",
    (
        ("dense", "self_attn.k_proj"),
        ("dense", "mlp.up_proj"),
        ("moe", "self_attn.v_proj"),
        ("moe", "mlp.experts.0.up_proj"),
    ),
)
@pytest.mark.parametrize("defect", ("input-width", "missing-sibling"))
def test_shared_oft_rejects_incompatible_or_incomplete_sibling_groups(
    tmp_path, architecture, sibling, defect
):
    shapes = _dense_shapes() if architecture == "dense" else _moe_shapes()
    name = f"model.layers.0.{sibling}"
    if defect == "input-width":
        inputs, outputs = shapes[name]
        shapes[name] = (inputs * 2, outputs)
    else:
        # Keep the suffix globally present; completeness is required per group.
        shapes[name.replace("layers.0.", "layers.1.")] = shapes.pop(name)
    with pytest.raises(FixtureValidationError, match="shared OFT"):
        build_oft_fixture(
            tmp_path / "bad",
            adapter_id="policy-a",
            architecture=architecture,
...
def test_matrix_is_exactly_the_four_active_h200_cells() -> None:
    ...
    assert len(cells) == 4
    assert {cell.precision for cell in cells} == {"bf16", "fp8"}
    assert {cell.gpu for cell in cells} == {"H200"}
    assert PINNED_MODEL_REVISIONS == EXPECTED_REVISIONS


def test_native_fixtures_include_embedding_and_lm_head(tmp_path: Path) -> None:
    oft = build_oft_fixture(
        tmp_path / "oft",
        adapter_id="policy-a",
        architecture="dense",
        seed=1729,
        target_shapes=_complete_dense_shapes(),
    )
    lora = build_lora_fixture(
        tmp_path / "lora",
        adapter_id="policy-a",
        architecture="dense",
        seed=1729,
    ...
    oft_tensors = load_file(oft.path / "adapter_model.safetensors")
    assert oft_tensors["base_model.model.model.embed_tokens.oft_R"].shape == (1, 8128)
    assert oft_tensors["base_model.model.lm_head.oft_R"].shape == (1, 8128)

    lora_tensors = load_file(lora.path / "adapter_model.safetensors")
    assert lora_tensors["base_model.model.model.embed_tokens.lora_A.weight"].shape == (
        8,
        256,
    )
    assert lora_tensors["base_model.model.model.embed_tokens.lora_B.weight"].shape == (
        128,
        8,
    )
    assert lora_tensors["base_model.model.lm_head.lora_A.weight"].shape == (
        8,
        128,
    )
    assert lora_tensors["base_model.model.lm_head.lora_B.weight"].shape == (
        256,
        8,
    )

    for fixture in (oft, lora):
        config = json.loads((fixture.path / "adapter_config.json").read_text())
        assert {"embed_tokens", "lm_head"} <= set(config["target_modules"])
...
def test_matrix_json_rejects_duplicate_object_keys(tmp_path: Path) -> None:
    ...
    duplicate.write_text(
        ...
        '"architecture":"dense","precision":"bf16","gpu":"H200",'
        '"tp":1,"ep":1}]\n'
    )
...
def test_prompt_manifest_has_fixed_cases_and_temperature_zero_batches() -> None:
    ...
    assert manifest.tokenizer_revision == "cdbee75f17c01a7cc42f958dc650907174af0554"
...
@pytest.mark.parametrize("cell_id,model", [(row[0], row[1]) for row in EXPECTED_CELLS])
def test_prompt_manifest_accepts_each_pinned_matrix_tokenizer(tmp_path, cell_id, model):
    records = [json.loads(line) for line in PROMPTS_PATH.read_text().splitlines()]
    records[0]["tokenizer"].update(model=model, revision=EXPECTED_REVISIONS[cell_id])
    path = tmp_path / "prompts.jsonl"
    path.write_text("".join(json.dumps(record) + "\n" for record in records))
    manifest = load_prompt_manifest(path)
    assert manifest.tokenizer_model == model
    assert manifest.tokenizer_revision == EXPECTED_REVISIONS[cell_id]
    assert manifest.prompts == load_prompt_manifest(PROMPTS_PATH).prompts


@pytest.mark.parametrize(
    "model,revision",
    [
        ("unknown/model", EXPECTED_REVISIONS["qwen3-4b-bf16"]),
        ("Qwen/Qwen3-30B-A3B", EXPECTED_REVISIONS["qwen3-4b-bf16"]),
        ("Qwen/Qwen3-30B-A3B", "main"),
    ],
)
def test_prompt_manifest_rejects_unknown_or_mismatched_tokenizer_pin(
    tmp_path, model, revision
):
    records = [json.loads(line) for line in PROMPTS_PATH.read_text().splitlines()]
    records[0]["tokenizer"].update(model=model, revision=revision)
    path = tmp_path / "prompts.jsonl"
    path.write_text("".join(json.dumps(record) + "\n" for record in records))
...
def _write_active_fake_snapshots(tmp_path: Path) -> dict[str, Path]:
    snapshots = {}
    for cell_id, *_ in EXPECTED_CELLS:
        revision = EXPECTED_REVISIONS[cell_id]
...
def test_preflight_hashes_config_index_every_shard_and_tokenizer(
    ...
    assert result.index_hash == _sha256(snapshot / "model.safetensors.index.json")
...
def test_preflight_hashes_exactly_one_unsharded_safetensors(
    ...
    assert result.files["model.safetensors"] == _sha256(snapshot / "model.safetensors")


def test_preflight_manifest_resolves_and_hashes_all_active_cells(
    tmp_path: Path,
) -> None:
    snapshots = _write_active_fake_snapshots(tmp_path)
...
def test_preflight_manifest_rejects_missing_and_mutable_cell_inputs(
    ...
    with pytest.raises(PreflightError, match="cannot override its pinned revision"):
        build_preflight_manifest(
            ...
            PROMPTS_PATH,
            snapshots,
            local_revisions={"qwen3-4b-bf16": "f" * 64},
        )
...
@pytest.mark.parametrize(
    "mutation", ("extra", "numbered-without-index", "indexed-single")
)
