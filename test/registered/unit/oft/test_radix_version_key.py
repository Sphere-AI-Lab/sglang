...
class TestReqCarriesVersion(unittest.TestCase):
    ...
    def test_same_adapter_different_version_keys_differ(self):
        r1 = self._req("r1", oft_id="A", oft_version=1)
        r2 = self._req("r2", oft_id="A", oft_version=2)
    ...
    def test_same_adapter_same_version_keys_match(self):
        a = self._req("r1", oft_id="A", oft_version=3)
        b = self._req("r2", oft_id="A", oft_version=3)
        self.assertEqual(a.extra_key, b.extra_key)

    def test_different_adapters_keys_differ(self):
        a = self._req("r1", oft_id="A", oft_version=1)
        b = self._req("r2", oft_id="B", oft_version=1)
    ...
    def test_version_is_stored_on_the_req(self):
        self.assertEqual(self._req("r1", oft_id="A", oft_version=9).oft_version, 9)
