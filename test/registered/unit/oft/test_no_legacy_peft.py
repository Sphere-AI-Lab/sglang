...
@pytest.mark.parametrize("suffix", [".md", ".mdx", ".jsx", ".py"])
def test_absence_guard_scans_product_documentation(tmp_path, monkeypatch, suffix):
    docs = tmp_path / "docs"
    docs.mkdir()
    (docs / ("adapter-example" + suffix)).write_text("--oft" + "-impl old")
    monkeypatch.setattr(sys.modules[__name__], "REPO_ROOT", tmp_path)
    with pytest.raises(AssertionError, match="Retired PEFT surface reintroduced"):
        test_legacy_peft_package_and_selectors_are_absent()
