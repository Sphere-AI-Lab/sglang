...
class _SentinelBackend:
    ...
    def run_gate_up_oft(self, x, R):
        return torch.cat([torch.ones_like(x), torch.full_like(x, 2)], dim=-1)


@pytest.fixture(autouse=True)
def _clear_global_fused_disable(monkeypatch):
    monkeypatch.delenv("SGLANG_OFT_DISABLE_FUSED_ROTATE_PROJECT", raising=False)
...
def test_tiny_gate_up_pool_keeps_fused_output(block_size):
    ...
    expected = torch.cat([torch.full_like(x, 11), torch.full_like(x, 13)], dim=-1)
    assert torch.equal(got, expected)
