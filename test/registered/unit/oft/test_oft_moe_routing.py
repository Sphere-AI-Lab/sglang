...
def _make_oft_wrapper():
    ...
    backend = TorchNativeOFTBackend(max_ofts_per_batch=3, device=torch.device("cpu"))
    moe_batch_info = _compute_moe_oft_info(
        ...
        oft_block_sizes=torch.tensor([0, 4, 4], dtype=torch.int32),
        max_ofts=3,
    )
...
def test_disk_expert_load_writes_only_selected_slot():
    pool = _expert_pool(
        {name: {0: torch.zeros(3, 2, 1, 4, 4)} for name in ("w1_oft_r", "w3_oft_r")}
    )
...
def test_expert_load_failure_does_not_publish_slot():
    ...
    # Lazy admission neutralizes the evicted slot before loading, then restores
    # it again after a potentially partial expert load fails.
    assert pool.reset_buffer_slot_to_identity.call_args_list == [call(2), call(2)]
...
def test_prepare_oft_batch_attaches_request_metadata(
    backend_cls, monkeypatch, block_sizes, enabled
):
    ...
    backend.prepare_oft_batch(
        ...
        oft_block_sizes=block_sizes,
        use_cuda_graph=False,
    )

    info = backend.batch_info.moe_oft_info
    assert backend.batch_info.has_active_oft is bool(any(enabled))
    assert info.seg_indptr.tolist() == [0, 2, 3, 5]
    assert info.req_to_oft.tolist() == [0, 1, 2]
    assert info.adapter_enabled.tolist() == enabled
    assert info.token_oft_mapping.tolist() == [0, 0, 1, 2, 2]
