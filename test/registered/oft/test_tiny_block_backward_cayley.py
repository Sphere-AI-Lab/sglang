...
from sglang.srt.oft.torch_ops.oft_ops import cayley_neumann as torch_cayley_neumann
from sglang.srt.oft.triton_ops.cayley_neumann import (
    cayley_neumann,
    cayley_neumann_bwd,
    cayley_neumann_fwd,
)
from sglang.srt.oft.triton_ops.gemm_oft_r_backward import gemm_oft_r_bwd
...
def test_backward_and_segmented_grad_r_match_torch(block_size):
    ...
    torch.testing.assert_close(grad_x.float(), expected_grad_x, atol=TOL, rtol=0)
    torch.testing.assert_close(grad_R.float(), expected_grad_R, atol=TOL, rtol=0)
