...
def _inputs(M, BS, device="cuda", dtype=torch.bfloat16, rotate=True, seed=0):
    ...
    W = (
        torch.randn(sum(OUT), K, device=device, dtype=dtype, generator=g) * 0.02
    ).contiguous()
    blocks = 3 * (K // BS)
    ...
    noise = (
        torch.randn(blocks, BS, BS, device=device, dtype=torch.float32, generator=g)
        * 0.02
    )
...
def _unfused_projection(x, R4, W, output_sizes, bias, slot, bsv):
    hidden = x.shape[-1]
    rotated = gemm_oft_r_fwd(x, R4, slot, bsv, num_slices=len(output_sizes))
...
def test_tiny_qkv_matches_unfused(BS, M, identity, with_bias):
    ...
    got = fused_rotate_project_qkv(x, R4, W, OUT, bias=bias, slot_idx_t=slot, bsv_t=bsv)
...
def test_tiny_qkv_runtime_identity_slot(BS):
    ...
    out = fused_rotate_project_qkv(x, R4, W, OUT, slot_idx_t=slot, bsv_t=bsv)
    torch.cuda.synchronize()
    err = (out.float() - (x.float() @ W.float().T)).abs().max().item()
    assert err <= TOL, f"BS={BS} max_abs={err:.2e}"
...
def test_tiny_qkv_cuda_graph_replays_identity_then_rotation(BS):
    ...
    with torch.cuda.stream(warmup_stream):
        fused_rotate_project_qkv(x, R4, W, OUT, slot_idx_t=slot, bsv_t=bsv)
    ...
    torch.cuda.synchronize()
    identity_error = (
        (captured_out.float() - (x.float() @ W.float().T)).abs().max().item()
    )
    ...
    torch.cuda.synchronize()
    rotation_error = (
        (captured_out.float() - _reference(x, R, W, OUT)).abs().max().item()
    )
...
def _fc1_inputs(M, BS, device="cuda", dtype=torch.bfloat16, seed=1):
    g = torch.Generator(device=device).manual_seed(seed)
    x = (
        torch.randn(M, FC1_K, device=device, dtype=dtype, generator=g) * 0.01
    ).contiguous()
    W = (
        torch.randn(sum(FC1_OUT), FC1_K, device=device, dtype=dtype, generator=g) * 0.02
    ).contiguous()
    blocks = 2 * (FC1_K // BS)
    eye = torch.eye(BS, device=device, dtype=dtype)
    noise = (
    ...
    R = (
        (eye.float().unsqueeze(0) + (noise - noise.transpose(-1, -2)))
        .to(dtype)
        .contiguous()
    )
    return x, R, W
...
def test_the_tile_picker_respects_the_shared_memory_budget():
    ...
    for BS in (4, 8, 128, 256, 512, 1024):
        for block_m, block_n, group_n in (
            (64, 64, 1),
            (64, 64, 2),
            (16, 64, 4),
            (32, 64, 8),
