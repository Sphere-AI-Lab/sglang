...
_GLOBAL_TARGETS = {"model.embed_tokens", "lm_head"}
...
def validate_matrix(
    ...
    """Load and validate the exact, ordered four-cell H200 matrix."""
    ...
    if ids != expected_ids:
        raise FixtureValidationError(
            "matrix order must match the binding four-cell H200 matrix"
        )
...
def _valid_target_name(name: str, architecture: str) -> bool:
    if name in _GLOBAL_TARGETS:
        return True
    target_pattern = _DENSE_TARGET if architecture == "dense" else _MOE_TARGET
    return target_pattern.fullmatch(name) is not None
...
def _target_shapes(
    ...
    *,
    ...
    for name in sorted(target_shapes):
        if not _valid_target_name(name, architecture):
            raise FixtureValidationError(f"unresolved target name: {name}")
        suffix = name.rsplit(".", 1)[-1]
        if name not in _GLOBAL_TARGETS and suffix not in allowed_suffixes:
            raise FixtureValidationError(f"unresolved target name: {name}")
        ...
        rotation_features = (
            output_features if name == "model.embed_tokens" else input_features
        )
        if block_size is not None and rotation_features % block_size:
            raise FixtureValidationError(
                f"target {name} rotation width must be divisible by block size "
                f"{block_size}"
            )
        suffixes.add(suffix)
...
def _shared_oft_targets(
    targets: tuple[tuple[str, int, int], ...],
) -> tuple[tuple[str, int, int], ...]:
    """Use one input rotation per fused module in native ``oft_type='oft'``.

    Expert checkpoint parsing accepts gate/up/down leaves, and the shared
    expert w13 buffer consumes gate_proj.oft_R for both gate and up.
    """
    shapes = {name: (inputs, outputs) for name, inputs, outputs in targets}
    groups = {
    ...
    for name, inputs, outputs in targets:
        ...
        siblings = groups.get(suffix)
        if siblings is None:
            resolved[name] = (inputs, outputs)
            continue
        names = [f"{parent}.{sibling}" for sibling in siblings]
        if any(sibling not in shapes for sibling in names):
            raise FixtureValidationError(
                f"shared OFT requires complete siblings: {parent}"
            )
        if len({shapes[sibling][0] for sibling in names}) != 1:
            raise FixtureValidationError(
                f"shared OFT sibling input widths disagree: {parent}"
            )
        fused = (
            "qkv_proj"
            if siblings[0] == "q_proj"
            else ("gate_proj" if ".experts." in parent else "gate_up_proj")
        )
        resolved[f"{parent}.{fused}"] = (
            inputs,
            sum(shapes[sibling][1] for sibling in names),
        )
...
def build_oft_fixture(
    ...
    *,
    ...
    """Create compact OFT; ``shared=True`` selects the native non-split layout.

    The default preserves independent split tensors for canonical unit coverage.
    """

    path, validated_seed = _validate_build_inputs(destination, adapter_id, seed)
    block_size = DENSE_OFT_BLOCK_SIZE if architecture == "dense" else MOE_OFT_BLOCK_SIZE
    ...
    if shared:
        targets = _shared_oft_targets(targets)
    ...
    for name, input_features, output_features in targets:
        ...
        tensors[tensor_name] = _deterministic_values(
            ...
            (rotation_features // block_size, compact_size),
            1e-3,
        )
    config = {
        "inference_mode": True,
