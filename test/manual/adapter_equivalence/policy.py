...
class NumericTolerance:
    ...
    @classmethod
    def create(cls, *, repetitions: tuple[BaselineRepetition, ...]) -> NumericTolerance:
    ...
    def validate(self, name: str) -> None:
        ...
        if len(self.repetitions) < 3:
            raise BundleValidationError(
                f"{context} requires at least three unchanged-baseline repetitions"
            )
...
class ToleranceEnvelope:
    ...
    def _validate_evidence(self) -> None:
        ...
        for name, tolerance in self.tolerances.items():
            if type(name) is not str or not name:
                raise BundleValidationError("tolerance names must be non-empty strings")
    ...
    def from_dict(cls, value: object) -> ToleranceEnvelope:
        ...
        envelope = cls(
            ...
            comparison_policy=ComparisonPolicy.from_dict(mapping["comparison_policy"]),
        )
        envelope.validate()
        return envelope
