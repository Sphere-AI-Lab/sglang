...
class DiscardAdapterStageReqInput(BaseReq, kw_only=True):
    """Internal rollback of one hidden stage; never receives tensor payloads."""
...
class DiscardAdapterStageReqOutput(BaseReq, kw_only=True):
    success: bool
    message: str
...
class CudaMemoryPeakRankResult(msgspec.Struct, kw_only=True):
    rank: int
    sample_id: str
...
class ResetCudaMemoryPeakReqInput(BaseReq, kw_only=True):
    # Validated before CUDA work; identity is preserved exactly, without trimming.
    sample_id: str


class ResetCudaMemoryPeakReqOutput(BaseReq, kw_only=True):
    sample_id: str
    operation: Literal["reset"]
    success: bool
    message: str
    ranks: List[CudaMemoryPeakRankResult]


class ReadCudaMemoryPeakReqInput(BaseReq, kw_only=True):
    sample_id: str


class ReadCudaMemoryPeakReqOutput(BaseReq, kw_only=True):
    sample_id: str
