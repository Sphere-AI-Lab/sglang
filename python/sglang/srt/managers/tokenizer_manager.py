...
class TokenizerManager(TokenizerControlMixin, TokenizerManagerScoreMixin):
    ...
    def _discard_pending_req_states(self, obj):
        ...
        for rid in rids:
            ...
            # A failed/partial dispatch may never produce a scheduler terminal
            # for this rid, and once the state is dropped a late terminal has no
            # release point either — so release here. Pre-acquire failures are
            # covered by the finalizer's lora_id check.
            self._finalize_lora_lease(state)
