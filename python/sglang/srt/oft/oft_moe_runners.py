...
def _compute_oft_alignment(
    ...
    max_num_tokens_padded = topk_ids.numel() + oft_info.num_experts * (block_size_m - 1)
...
def _oft_prerotate(
    ...
    # Routing and request boundaries both use original-token coordinates.
    # Alignment already emits flattened token/expert IDs, including for down
    # inputs with expanded rows; scaling boundaries here overruns its mask.
    (
