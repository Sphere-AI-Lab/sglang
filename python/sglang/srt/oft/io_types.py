...
class OFTUpdateOutput(BaseReq, kw_only=True):
    ...
    # True only when a failed upsert verified that the requested prior
    # identity/version is still safe to serve on this worker.
    previous_adapter_preserved: bool = False
    # A rank may have installed new state while peers rejected the update.
    # Preserve this through aggregation so fresh loads are quarantined too.
