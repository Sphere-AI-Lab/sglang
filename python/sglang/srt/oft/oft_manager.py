...
from sglang.srt.oft.layers import (
    BaseLayerWithOFT,
...
def _raise_streamed_expert_oft_buffer_mismatch(
    *,
    ...
    expected_buffer_dtype: torch.dtype,
) -> None:
    message = (
        ...
        f"expected_buffer_dtype={expected_buffer_dtype}. "
...
class OFTManager(AdapterManager):
    ...
    def unload_oft_adapter(self, oft_ref: OFTRef) -> "OFTUpdateOutput":
        try:
            pending_event = self.pending_oft_load_events.pop(oft_ref.adapter_id, None)
            if pending_event is not None:
                pending_event.synchronize()
        except Exception as exc:
            return self.create_oft_update_result(success=False, error_message=str(exc))
        return self.unload_adapter(oft_ref)

    def unload_adapter(self, ref: OFTRef) -> "OFTUpdateOutput":
        if ref.adapter_id not in self.adapters:
            return super().unload_adapter(ref)
        # CPU-backed adapters own resident slots after their first serving
        # batch. Release that state before dropping the CPU backing; a reset
        # failure must leave the registration available for an unload retry.
        stored_ref = self.refs.get(ref.adapter_id, ref)
        result = self.unload_streamed_adapter(stored_ref)
        if result.success:
            self.adapters.pop(ref.adapter_id, None)
        return result
    ...
    def unload_streamed_adapter(self, oft_ref: OFTRef) -> "OFTUpdateOutput":
        """Release resident buffers and registration without touching disk backing.

        CPU-backed unloads remove their OFTAdapter object only after this
        cleanup succeeds. GPU-only legacy registrations use this directly.
        """
    ...
    def _is_expected_previous_streamed_ref(
        self, old_ref: OFTRef, new_ref: OFTRef
    ) -> bool:
        identity_matches = (
            old_ref.adapter_id == new_ref.adapter_id
            and old_ref.adapter_name == new_ref.adapter_name
            and old_ref.adapter_version + 1 == new_ref.adapter_version
        )
        active_versions = getattr(self.memory_pool, "_active_versions", None)
        active_version = (
            active_versions.get(old_ref.adapter_id)
            if active_versions is not None
            else None
        )
        return identity_matches and (
            active_version is None or active_version == old_ref.adapter_version
        )

    def _try_transactional_streamed_upsert(
        self,
        ref: OFTRef,
        named_tensors,
        plan,
        ...
        config_dict: dict,
        existing_id: str,
    ) -> Optional["OFTUpdateOutput"]:
        """Rewrite a resident wire adapter with the hidden slot as rollback.

        Returns ``None`` when the memory pool has no hidden staging slot, so
        legacy/non-staged manager fixtures retain their existing path.
        """
        memory_pool = self.memory_pool
        staging_idx = getattr(memory_pool, "staging_idx", None)
        copy_slot = getattr(memory_pool, "copy_supported_buffer_slot", None)
        if (
            staging_idx is None
            or staging_idx < memory_pool.max_ofts_per_batch
            or copy_slot is None
        ):
            return None

        old_ref = self.refs[existing_id]
        if ref.adapter_id != existing_id:
            return self._make_update_result(
                success=False,
                error_message=(
                    f"OFT upsert for '{ref.adapter_name}' must reuse adapter_id "
                    f"{existing_id}, got {ref.adapter_id}."
                ),
            )
        if not self._is_expected_previous_streamed_ref(old_ref, ref):
            return self._make_update_result(
                success=False,
                error_message=(
                    f"OFT upsert for '{ref.adapter_name}' expected worker version "
                    f"{ref.adapter_version - 1}, but found "
                    f"{old_ref.adapter_version}; restart required."
                ),
            )

        staged_identity = getattr(memory_pool, "staged_identity", lambda: None)()
        if staged_identity is not None:
            return self._make_update_result(
                success=False,
                error_message=(
                    "Cannot upsert an OFT adapter while the hidden staging slot "
                    f"holds uid={staged_identity[0]} version={staged_identity[1]}."
                ),
                previous_adapter_preserved=True,
            )
        buffer_id = memory_pool.uid_to_buffer_id.get(existing_id)
        if buffer_id is None:
            return self._make_update_result(
                success=False,
                error_message=(
                    f"Wire-loaded OFT adapter '{ref.adapter_name}' has no resident "
                    "buffer to update safely."
                ),
            )

        try:
            new_config = OFTConfig.from_dict(config_dict)
        ...
        try:
            ...
            copy_slot(buffer_id, staging_idx)
            if self.device.type == "cuda":
                torch.cuda.synchronize(self.device)
        except Exception as error:
            return self._make_update_result(
                success=False,
                error_message=f"Failed to back up the existing OFT adapter: {error}",
                previous_adapter_preserved=True,
            )
        ...
        try:
            memory_pool.reset_buffer_slot_to_identity(buffer_id)
        ...
        if not success:
            previous_adapter_preserved = True
            try:
                copy_slot(staging_idx, buffer_id)
                if self.device.type == "cuda":
                    torch.cuda.synchronize(self.device)
            except Exception as restore_error:
                previous_adapter_preserved = False
                cleanup_result = self.unload_streamed_adapter(old_ref)
                cleanup_error = (
                    ""
                    if cleanup_result.success
                    else f"; cleanup also failed: {cleanup_result.error_message}"
                )
                error_message = (
                    f"{error_message}; restoring the previous adapter failed: "
                    f"{restore_error}; worker restart required{cleanup_error}"
                )
            finally:
                try:
                    memory_pool.reset_buffer_slot_to_identity(staging_idx)
                except Exception:
                    logger.exception(
                        "Failed to reset OFT rollback slot %s after upsert failure",
                        staging_idx,
                    )
            return self._make_update_result(
                success=False,
                error_message=error_message,
                previous_adapter_preserved=previous_adapter_preserved,
            )

        old_pinned = int(bool(old_ref.pinned))
        self.configs[existing_id] = new_config
        self.refs[existing_id] = ref
        self.num_pinned += int(bool(ref.pinned)) - old_pinned
        active_versions = getattr(memory_pool, "_active_versions", None)
        if active_versions is not None:
            active_versions[existing_id] = ref.adapter_version
        memory_pool.eviction_policy.mark_used(existing_id)
        try:
            memory_pool.reset_buffer_slot_to_identity(staging_idx)
        except Exception:
            logger.exception(
                "Failed to reset OFT rollback slot %s after successful upsert",
                staging_idx,
            )
    ...
    def load_adapter_from_tensors(
        ...
        """Retain native streamed weights on CPU; admit GPU slots on demand.

        Like LoRA, a fresh load does not evict an unrelated running adapter.
        A resident upsert updates its slot transactionally before publishing the
        new CPU snapshot. Nonresident updates only replace the CPU snapshot.
        """
        from sglang.srt.oft.streamed_weight_loader import (
        ...
        existing_ref = next(
            (old for old in self.refs.values() if old.adapter_name == ref.adapter_name),
            None,
        )
        previous_adapter_preserved = bool(
            upsert
            and existing_ref is not None
            and self._is_expected_previous_streamed_ref(existing_ref, ref)
        )
        try:
            if existing_ref is not None:
                if not upsert:
                    raise ValueError(
                        f"OFT adapter '{ref.adapter_name}' is already loaded; "
                        "pass upsert=True to refresh it in place."
                    )
                if existing_ref.reloadable:
                    raise ValueError(
                        "Cannot replace a disk-loaded OFT adapter via wire upsert"
                    )
                if not previous_adapter_preserved:
                    raise ValueError(
                        "OFT upsert must retain identity and advance the active version"
                    )

            pinned_count = (
                self.num_pinned
                + int(bool(ref.pinned))
                - int(bool(existing_ref and existing_ref.pinned))
            )
            if ref.pinned and pinned_count >= self.memory_pool.max_ofts_per_batch:
                raise ValueError(
                    "Pinned OFT adapters must leave a serving slot for unpinned/base requests"
                )

            block_size = config_dict.get("oft_block_size", 32)
            if block_size != self.memory_pool.max_oft_block_size:
                raise ValueError(
                    f"OFT adapter block_size={block_size} does not match "
                    f"--max-oft-block-size={self.memory_pool.max_oft_block_size}"
                )
            new_config = OFTConfig.from_dict(config_dict)
            new_adapter = OFTAdapter(
                ref.adapter_id,
                new_config,
                self.base_hf_config,
                self.load_config,
            ...
            new_adapter.initialize_weights_from_tensors(dict(named_tensors))
            payload = new_adapter.streamed_named_tensors
            plan, error = _resolve_streamed_oft_tensor_groups(self, payload, block_size)
            if plan is None:
                raise ValueError(error)

            if (
                existing_ref is not None
                and ref.adapter_id in self.memory_pool.uid_to_buffer_id
            ):
                result = self._try_transactional_streamed_upsert(
                    ref, payload, plan, block_size, config_dict, ref.adapter_id
                )
                if result is not None:
                    if result.success:
                        self.adapters[ref.adapter_id] = new_adapter
                    return result

                # Pools without a staging row restore from the retained CPU
                # snapshot if refreshing the resident slot fails.
                old_adapter = self.adapters[ref.adapter_id]
                slot = self.memory_pool.uid_to_buffer_id[ref.adapter_id]
                if self.device.type == "cuda":
                    torch.cuda.synchronize(self.device)
                try:
                    self.memory_pool.reset_buffer_slot_to_identity(slot)
                    self._restore_streamed_oft(new_adapter, slot)
                except Exception as update_error:
                    try:
                        self.memory_pool.reset_buffer_slot_to_identity(slot)
                        self._restore_streamed_oft(old_adapter, slot)
                    except Exception as restore_error:
                        previous_adapter_preserved = False
                        raise RuntimeError(
                            f"{update_error}; rollback failed: {restore_error}; restart required"
                        ) from restore_error
                    raise

            self.configs[ref.adapter_id] = new_config
            self.adapters[ref.adapter_id] = new_adapter
            self.refs[ref.adapter_id] = ref
            self.num_pinned += int(bool(ref.pinned)) - int(
                bool(existing_ref and existing_ref.pinned)
            )
            active_versions = getattr(self.memory_pool, "_active_versions", None)
            if active_versions is not None:
                active_versions[ref.adapter_id] = ref.adapter_version
        except Exception as error:
            return self._make_update_result(
                success=False,
                error_message=str(error),
    ...
    def _restore_streamed_oft(
        self, adapter: OFTAdapter, slot: int, *, adapter_name: Optional[str] = None
    ):
        """Use native tensor routing for every CPU-backed slot restoration."""
        from sglang.srt.oft.streamed_weight_loader import (
            _commit_streamed_oft_tensor_groups,
            _resolve_streamed_oft_tensor_groups,
        )

        payload = adapter.streamed_named_tensors
        plan, error = _resolve_streamed_oft_tensor_groups(
            self, payload, adapter.block_size
        )
        if plan is None:
            raise ValueError(error)
        success, error = _commit_streamed_oft_tensor_groups(
            ...
            (
                ...
                else self.refs[adapter.uid].adapter_name
            ),
            adapter.uid,
        )
        if not success:
    ...
    def load_adapter_from_distributed(
        ...
        *,
        ...
        try:
            ...
            previous_adapter_preserved = upsert and any(
                self._is_expected_previous_streamed_ref(existing_ref, ref)
        ...
        return self.load_adapter_from_tensors(ref, tensors, config_dict, upsert=upsert)
    ...
    def _prepare_mem_pool_batch(self, cur_uids):
        self.memory_pool.prepare_oft_batch(
            ...
            streamed_loader=self._restore_streamed_oft,
        )
        ...
        for module_name, module in self.base_model.named_modules():
            ...
            if module_suffix in self.target_modules:
                ...
                if layer_id is None:
                    ...
                    continue
                oft_module = self.set_oft_module(module_name, module)
                if module_suffix == "fused_qkv_a_proj_with_mqa" and isinstance(
                    oft_module, ReplicatedLinearWithOFT
                ):
                    oft_module.first_output_dim = (
                        _get_hf_config_attr(self.base_hf_config, "q_lora_rank") or 0
                    )
                self.adapter_modules[layer_id][module_name] = oft_module
                wrapped_modules.append(module_name)
    ...
    def _apply_expert_oft_to_module(self, moe, ew_dict, block_size, layer_id, slot_idx):
        ...
        if gate_sample is None and up_sample is None and down_sample is None:
            return
        # The pool layout is fixed at startup. A partial canonical checkpoint
        # can contain only gate or only up without becoming a shared-R adapter.
        is_split = self.oft_type == "canonical_oft"
        if not is_split and up_sample is not None:
            raise ValueError(
        ...
        w13_sample = gate_sample if gate_sample is not None else up_sample
        oft_sample = w13_sample if w13_sample is not None else down_sample
        dtype = oft_sample.dtype

        num_blocks_w13 = w13_sample.shape[0] if w13_sample is not None else 0
        ...
        # These complete groups replace the selected slot below. Experts and
        # projections absent from the checkpoint must remain neutral.
        for rotation in (w1_oft_r, w3_oft_r, w2_oft_r):
            if rotation is not None:
                _fill_expert_oft_identity(rotation)
        ...
        def _batched_cayley_assign(out, compacts, num_blocks):
            ...
            if not valid_idx:
                return
            stacked = torch.cat(
                [compacts[i].to(device=device, dtype=dtype) for i in valid_idx],
                dim=0,
            )
    ...
    def apply_streamed_expert_oft(self, expert_tensors, block_size, slot_idx=None):
        ...
        for layer_id, ew_dict in expert_tensors.items():
            ...
            if gate_sample is None and up_sample is None and down_sample is None:
                continue
            # Gate and up may arrive in separate chunks. Routing each chunk
            # must follow the configured pool layout, not its tensor contents.
            is_split = self.oft_type == "canonical_oft"
            if not is_split and up_sample is not None:
                raise ValueError(
                    "Independent expert up_proj OFT rotations require canonical_oft layout"
            ...
            # Compact dtype controls Cayley precision; resident rotations use
            # the configured pool dtype. Reuse storage to preserve captured
            # addresses and experts filled by earlier chunks in this sync.
            ...
            def _validate_w13_buffer(buf, projection):
                if num_blocks_w13 > 0 and not (
                    ...
                    and buf.dtype == self.oft_r_dtype
                    ...
                    _raise_streamed_expert_oft_buffer_mismatch(
                        ...
                        expected_buffer_dtype=self.oft_r_dtype,
                    )
                if num_blocks_w13 == 0:
                    return None
                return buf
            ...
            if is_split:
                w1_oft_r = (
                    _validate_w13_buffer(_resolve_expert_buffer("w1_oft_r"), "w1")
                    if gate_sample is not None
                    else None
                )
                w3_oft_r = (
                    _validate_w13_buffer(_resolve_expert_buffer("w3_oft_r"), "w3")
                    if up_sample is not None
                    else None
                )
                w13_oft_r = None
            elif gate_sample is not None:
            ...
            if blocks_per_tp > 0 and not (
                ...
                and w2_oft_r.dtype == self.oft_r_dtype
            ...
            for global_id, ew in ew_dict.items():
                ...
                if gate_compact is not None:
                    gate_compacts.append(gate_compact.to(device=device, dtype=dtype))
                else:
                    gate_compacts.append(None)

                up_compact = ew.get("up_proj.oft_R")
                if up_compact is not None:
                    up_compacts.append(up_compact.to(device=device, dtype=dtype))
                else:
            ...
            def _scatter(buf, compacts, num_blocks):
                ...
                # Match disk loading: compute in checkpoint precision, then
                # cast the final rotation into the existing resident buffer.
                R_stacked = R_stacked.to(dtype=buf.dtype)
                R_per = R_stacked.view(len(valid), num_blocks, block_size, block_size)
    ...
    def _bump_ref_version(self, name, version):
        for adapter_id, ref in self.refs.items():
            if ref.adapter_name == name:
                ...
                return
        raise ValueError(f"OFT adapter {name!r} not found in refs; cannot bump version")
