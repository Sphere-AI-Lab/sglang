...
def maybe_init_oft_manager(model_runner: ModelRunner, server_args: ServerArgs) -> None:
...
def _init_oft_manager(model_runner: ModelRunner, server_args: ServerArgs) -> None:
    ...
    if (
        ...
        and model_runner.oft_manager.oft_backend.is_moe_oft
    ):
        raise ValueError(
            "NGRAM speculative decoding is not supported with MoE OFT. "
...
def maybe_admit_request(scheduler: Scheduler, req: Req, running_ofts) -> bool:
    ...
    return scheduler.tp_worker.model_runner.oft_manager.validate_oft_batch(new_oft_set)
...
def maybe_extend_extra_key(extra_key, adapter_id, adapter_version) -> str:
    ...
    if adapter_id is not None:
        extra_key = (
            (extra_key or "")
            + f"|oft:{adapter_id}:v{0 if adapter_version is None else adapter_version}"
        )
    return extra_key
