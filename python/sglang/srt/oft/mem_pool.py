...
from sglang.srt.oft.base.mem_pool import EmptySlot as EmptySlot
...
# params mapping used by native LoRA, kept local here because the OFT
...
def get_stacked_multiply(module_name: str) -> int:
    """Return a fused projection's output stacking factor."""
    stacked_rank = {
        "qkv_proj": 3,
        "in_proj_qkvz": 4,
        "in_proj_ba": 2,
    ...
    return stacked_rank.get(module_name.rsplit(".", 1)[-1], 1)
    ...
    *,
    ...
    for key, tensor in weights.items():
        ...
        for fused_leaf, split_leaves in MERGED_OFT_PROJ_GROUPS.items():
            if (
                available_fused_targets is not None
                and fused_leaf not in available_fused_targets
            ):
                continue
...
def _broadcast_legacy_single_R(
    compact: torch.Tensor, stacked_multiply: int
) -> torch.Tensor:
...
def _fill_identity(buffer_view: torch.Tensor, block_size: int) -> None:
    ...
    if block_size <= 0:
        return
    eye = torch.eye(block_size, dtype=buffer_view.dtype, device=buffer_view.device)
...
def _write_oft_r_block(
    ...
    if slice_index is not None:
        if split_count <= 1:
            raise ValueError(f"slice_index={slice_index} requires split_count > 1")
...
class OFTMemoryPool(AdapterMemPool):
    ...
    def __init__(
        ...
        self.num_layer: int = get_hf_config_attr(base_hf_config, "num_hidden_layers")
        ...
        # Single global split(canonical)-vs-fused signal
        # (sglang.srt.oft.config.OFTArgs.oft_type); drives both dense buffer
        # stacking and the MoE split (w1_oft_r+w3_oft_r) vs fused (w13_oft_r)
        # group layout.
        ...
        # where num_blocks = r_dim // block_size. For canonical_oft the
        # multiplier follows the fused target (e.g. 3 for qkv, 2 for gate/up);
        # oft uses one shared input rotation, so its multiplier is always 1.
    ...
    def get_oft_R_shape(
        ...
        Fused targets store one shared input R for oft, or one R per output
        slice stacked along the blocks dimension for canonical_oft.
        """
        ...
        stacked_multiply = (
            get_stacked_multiply(module_name) if self.oft_type == "canonical_oft" else 1
        )
        shape = (
    ...
    def prepare_oft_batch(
        ...
        streamed_loader: Optional[Callable[[OFTAdapter, int], None]] = None,
    ):
        ...
        for uid in sorted(cur_uids, key=lambda uid: (uid is not None, uid or "")):
            if uid not in self.uid_to_buffer_id:
                buffer_id = self._acquire_buffer_slot(cur_uids, oft_refs, oft_adapters)
                self.reset_buffer_slot_to_identity(buffer_id)
                oft_adapter = oft_adapters.get(uid, None)
                try:
                    if (
                        oft_adapter is not None
                        and getattr(oft_adapter, "streamed_named_tensors", None)
                        is not None
                    ):
                        if streamed_loader is None:
                            raise RuntimeError(
                                "Streamed OFT restoration requires its loader"
                            )
                        streamed_loader(oft_adapter, buffer_id)
                    else:
    ...
    def load_oft_weight_to_buffer(
        ...
        for layer_id in range(self.num_layer):
            ...
            for name, weights in temp_R_buffer.items():
                ...
                self._precompute_and_store_R(
                    target_buffer[buffer_id], weights, block_size
                )

        self.load_oft_optional_weights_to_buffer(buffer_id, oft_adapter)
    ...
    def _precompute_and_store_R(
        self,
        buffer_view: torch.Tensor,
        compact_weight: Optional[torch.Tensor],
        block_size: int,
    ) -> None:
        """Precompute R from compact weights and store in a buffer view.

        Instead of storing compact 2D weights, precompute the full orthogonal
        rotation matrices via Cayley transform and store the 3D result. When
        ``compact_weight`` is absent, fill the block-diagonal identity rotation,
        the only OFT passthrough that leaves the input unchanged.
        """
    ...
    def load_oft_optional_weights_to_buffer(
        self, buffer_id: int, oft_adapter: OFTAdapter
    ) -> None:
        self.reset_optional_buffer_slot(buffer_id)
    ...
    def load_oft_weight_direct(
        ...
        fused_target, slice_module, _, slice_index, split_count = (
            self._resolve_oft_tensor_plan(
                name,
                oft_modules,
                layer_id,
            )
    ...
    def allocate_buffer_slot(self) -> int:
        ...
        uid_summary = ", ".join(
            f"{uid!r}->buffer[{bid}]" for uid, bid in self.uid_to_buffer_id.items()
        )
        logger.error(
    ...
    def reset_optional_buffer_slot(self, buffer_id: int) -> None:
        bs = self.max_oft_block_size
        for tensor in self.embedding_R_buffer.values():
            _fill_identity(tensor[buffer_id], bs)
        for tensor in self.lm_head_R_buffer.values():
            _fill_identity(tensor[buffer_id], bs)

    def copy_supported_buffer_slot(self, source: int, destination: int) -> None:
        for keyed in self._groups.values():
            for tensor in keyed.values():
