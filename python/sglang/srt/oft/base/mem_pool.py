...
class AdapterMemPool:
    ...
    def _acquire_buffer_slot(self, cur_uids, refs, cpu_backed_uids=()):
        ...
        for buffer_id in range(self.max_adapters_per_batch):
            ...
            if uid is not None:
                ref = refs.get(uid)
                if ref and (
                    ref.pinned or (not ref.reloadable and uid not in cpu_backed_uids)
                ):
                    continue
            candidates.add(uid)
        ...
        if not candidates:
            raise ValueError(
                ...
                "pinned adapters and adapters without CPU or disk backing is less "
