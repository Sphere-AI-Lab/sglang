...
class AdapterManager:
    ...
    def _make_update_result(
        self, success, error_message="", *, previous_adapter_preserved=False
    ):
    ...
    def load_adapter(self, ref):
        assert (
            ref.adapter_name is not None and ref.adapter_path is not None
        ), "Adapter ref must have both name and path set for loading."
        assert (
            ref.adapter_id not in self.adapters
    ...
    def unload_adapter(self, ref):
        ...
        if (
            adapter is None
            and stored_ref is None
            and ref.adapter_id not in self.adapters
        ):
            # A retry must succeed on ranks that completed an earlier
            # distributed unload. The tokenizer admits only active identities
            # or failed-unload tombstones to this internal operation.
            return self._make_update_result(success=True)
        stored_ref = stored_ref or ref
        ...
        try:
            ...
            self.configs.pop(ref.adapter_id, None)
            self.adapters.pop(ref.adapter_id, None)
            removed_ref = self.refs.pop(ref.adapter_id, None)
            if removed_ref is not None:
                self.num_pinned -= int(removed_ref.pinned)
        except Exception as e:
            return self._make_update_result(success=False, error_message=str(e))
        return self._make_update_result(success=True)
    ...
    def validate_batch(self, adapter_ids):
        ...
        # Pinned adapters reserve capacity even when absent from this batch.
        # Legacy GPU-only residents cannot be restored. CPU-backed streamed
        # adapters can be evicted even though they have no reloadable disk path.
        pinned_in_batch = sum(
            bool(self.refs[uid].pinned) for uid in adapter_ids if uid is not None
        )
        nonreloadable_outside_batch = sum(
            not ref.pinned and not ref.reloadable and uid not in self.adapters
            for uid in self.memory_pool.uid_to_buffer_id
            if uid is not None
            and uid not in adapter_ids
            and (ref := self.refs.get(uid)) is not None
        )
        required_slots = (
            len(adapter_ids)
            + self.num_pinned
            - pinned_in_batch
            + nonreloadable_outside_batch
        )
        return required_slots <= self.memory_pool.max_adapters_per_batch
