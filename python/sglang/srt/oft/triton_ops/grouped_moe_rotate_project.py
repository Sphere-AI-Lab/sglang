...
def _split_w13_oft_grouped_moe_kernel(
    ...
    if BLOCK_SIZE >= 16:
        for block_idx in range(0, BLOCKS):
            ...
            x_rot = tl.dot(x, r, input_precision="ieee", out_dtype=tl.float32).to(
                tl.bfloat16
            )
            w = tl.load(
                w13_ptr
...
def _pack_split_oft_grouped_bmm_inputs_kernel(
    ...
    r_base = (
        expert * BLOCKS * BLOCK_SIZE * BLOCK_SIZE + block_idx * BLOCK_SIZE * BLOCK_SIZE
    )
    ...
    if BLOCK_SIZE >= 16:
        ...
        packed_offsets = (
            packed_batch * MAX_PADDED_TOKENS_PER_EXPERT + rank_offsets[:, None]
        ) * K + k_offsets[None, :]
        tl.store(
        ...
        for j in range(BLOCK_SIZE):
            ...
            for k in range(BLOCK_SIZE):
                ...
                if half_id == 0:
                    r_kj = tl.load(w1_oft_r_ptr + r_base + k * BLOCK_SIZE + j).to(
                        tl.float32
                    )
                else:
                    r_kj = tl.load(w3_oft_r_ptr + r_base + k * BLOCK_SIZE + j).to(
                        tl.float32
                    )
                x_rot_j += x_k * r_kj
...
def fused_split_w13_oft_grouped_moe(
    *,
    ...
    if hidden_from_w != hidden:
        raise RuntimeError(
            f"w13 hidden dim {hidden_from_w} != hidden_states dim {hidden}"
        )
    if n % 2 != 0:
    ...
    if w1_oft_r.shape[-2] != w1_oft_r.shape[-1]:
        raise RuntimeError(
            f"OFT blocks must be square, got {tuple(w1_oft_r.shape[-2:])}"
        )
    ...
    if blocks * block_size != hidden:
        raise RuntimeError(
            f"OFT blocks {blocks} * block_size {block_size} != hidden {hidden}"
        )
    if block_m <= 0:
    ...
    out = torch.zeros(
        (m, top_k, n), device=hidden_states.device, dtype=hidden_states.dtype
    )
...
def _make_grouped_bmm_expert_offsets(
    *,
    ...
    if max_padded_tokens_per_expert <= 0:
        raise RuntimeError(
            "Grouped-BMM experiment needs at least one local expert block"
        )
...
def packed_bmm_split_w13_oft_grouped_moe(
    *,
    ...
    if (
        w13_grouped_bmm_weight is not None
        and not w13_grouped_bmm_weight.is_contiguous()
    ):
...
def _make_benchmark_inputs(
    *,
    ...
    w13 = (
        torch.randn(experts, half * 2, hidden, device="cuda", dtype=torch.bfloat16)
        * 0.02
    )
...
def _run_benchmark(json_path: Optional[str] = None) -> int:
    ...
    header = f"{'BS':>4} {'M':>6} {'variant':>12} {'us':>11} {'max_abs':>10}  note"
    ...
    for block_size in block_sizes:
        for m in ms:
            ...
            for variant, (out, us, error) in measured.items():
                ...
                err_text = f"{max_abs:10.5f}" if max_abs is not None else f"{'--':>10}"
                print(
