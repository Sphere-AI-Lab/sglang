...
def _grad_R_kernel(
    ...
    for t_start in range(0, total_tokens, TILE_T):
        ...
        gy_tile = tl.load(
            grad_y_ptr
            + t_offsets[:, None] * grad_y_stride_0
            + col_base_gy
            + c_offsets[None, :],
            mask=t_mask[:, None],
            other=0.0,
    ...
    out_base = (
        grad_R_ptr + adapter_idx * grad_R_stride_0 + weight_block_idx * grad_R_stride_1
    )
    tl.store(
        out_base
        + k_offsets[:, None] * grad_R_stride_2
        + c_offsets[None, :] * grad_R_stride_3,
...
def gemm_oft_r_bwd_grad_R(
    ...
    if block_size_2 != block_size:
        raise ValueError(
            f"weights_shape block dims must be square, got {weights_shape}"
        )
