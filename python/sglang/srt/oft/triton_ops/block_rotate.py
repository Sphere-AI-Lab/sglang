...
def _oft_block_rotate_kernel(
    ...
    # Bounds check
    num_tokens_post_padded = tl.load(num_tokens_post_padded_ptr + slot_id * stride_ntpp)
    ...
    expert = tl.load(expert_ids_ptr + slot_id * stride_eis + pid_m * stride_eim).to(
        tl.int64
    )
    ...
    if OFT_BLOCK_SIZE >= 16:
        for k_off in range(0, OFT_BLOCK_SIZE, TILE_K):
            ...
            a_ptrs = (
                A_ptr + orig_ids[:, None] * stride_am + k_tile_offs[None, :] * stride_ak
            )
    ...
    out_ptrs = (
        A_rot_ptr + sorted_ids[:, None] * stride_arm + out_k_offs[None, :] * stride_ark
    )
    tl.store(
        out_ptrs, rot_accum.to(A_rot_ptr.dtype.element_ty), mask=token_mask[:, None]
    )
...
def apply_oft_rotation_triton(
    ...
    _oft_block_rotate_kernel[grid](
        ...
        sorted_token_ids.stride(1),
        expert_ids,
        expert_ids.stride(0),
        expert_ids.stride(1),
        num_tokens_post_padded,
        num_tokens_post_padded.stride(0),
        oft_ids,
        adapter_enabled,
        topk_ids.numel(),
        top_k=top_k,
