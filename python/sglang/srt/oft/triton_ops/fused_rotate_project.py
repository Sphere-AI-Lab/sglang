...
def _fused_rotate_project_qkv_kernel(
    ...
    S0: tl.constexpr,
    S1: tl.constexpr,
    S2: tl.constexpr,  # per-slice widths
    BS: tl.constexpr,
    ...
    if BS >= 16:
        for j in range(0, BS, TILE_K):
            ...
            if bsv == 0:
                ...
                tl.store(
                    out_gate_ptr + offs_m[:, None] * K + offs_kj[None, :],
                    x_j,
                    mask=m_mask[:, None],
                )
                tl.store(
                    out_up_ptr + offs_m[:, None] * K + offs_kj[None, :],
                    x_j,
                    mask=m_mask[:, None],
                )
            else:
...
def _fused_rotate_project_inner(
    ...
    if BS < 16:
        ...
        for block_group in range(0, blocks_per_slice, blocks_in_tile):
            ...
            w0 = tl.load(
                W_ptr + (slice_offset + offs_n0)[:, None] * K + offs_k[None, :],
            ...
            if GROUP_N >= 2:
                w1 = tl.load(
                    W_ptr + (slice_offset + offs_n1)[:, None] * K + offs_k[None, :],
            ...
            if GROUP_N >= 4:
                w2 = tl.load(
                    W_ptr + (slice_offset + offs_n2)[:, None] * K + offs_k[None, :],
                    mask=n_mask2[:, None] & valid_blocks[None, :],
                    other=0.0,
                )
                w3 = tl.load(
                    W_ptr + (slice_offset + offs_n3)[:, None] * K + offs_k[None, :],
            ...
            if GROUP_N >= 8:
                w4 = tl.load(
                    W_ptr + (slice_offset + offs_n4)[:, None] * K + offs_k[None, :],
                    mask=n_mask4[:, None] & valid_blocks[None, :],
                    other=0.0,
                )
                w5 = tl.load(
                    W_ptr + (slice_offset + offs_n5)[:, None] * K + offs_k[None, :],
                    mask=n_mask5[:, None] & valid_blocks[None, :],
                    other=0.0,
                )
                w6 = tl.load(
                    W_ptr + (slice_offset + offs_n6)[:, None] * K + offs_k[None, :],
                    mask=n_mask6[:, None] & valid_blocks[None, :],
                    other=0.0,
                )
                w7 = tl.load(
                    W_ptr + (slice_offset + offs_n7)[:, None] * K + offs_k[None, :],
...
def _pick_tile_k(
    BS: int, block_m: int, block_n: int, group_n: int, preferred: int = OFT_TILE_K
) -> int:
    ...
    while (
        tile_k > 16
        and _tiled_smem_bytes(tile_k, block_m, block_n, group_n) > OFT_SMEM_BUDGET
    ):
        tile_k //= 2
    return tile_k
...
def _validate_inputs(x, R, W, output_sizes, num_slices_expected):
    ...
    assert (
        num_slices == num_slices_expected
...
def fused_rotate_gate_up_inputs(
    ...
    *,
    ...
    tile_k_sel = _pick_tile_k(BS, BLOCK_M, BLOCK_M, 2, preferred=tile_k or OFT_TILE_K)
...
def fused_rotate_project_qkv(
    ...
    *,
    ...
    tile_k = _pick_tile_k(BS, BLOCK_M, BLOCK_N, GROUP_N, preferred=tile_k or OFT_TILE_K)
...
def fused_rotate_project_gate_up(
    ...
    *,
    ...
    tile_k_sel = _pick_tile_k(
        BS, BLOCK_M, BLOCK_N, GROUP_N, preferred=tile_k or OFT_TILE_K
    )
...
if __name__ == "__main__":
    ...
    WRAPPERS = [
        (
            "QKV",
            {"K": 3584, "output_sizes": [448, 64, 64], "block_size": 32},
            fused_rotate_project_qkv,
        ),
        (
            "FC1",
            {"K": 3584, "output_sizes": [2368, 2368], "block_size": 32},
    ...
    BSV_REGIMES = [
        (
            "identity",
            0,
        ),  # bsv=0 → skip rotation; captures the capture-time / KL forward case
    ...
    print(
        "Baselines: 'legacy' = gemm_oft_r_fwd + per-slice F.linear + cat (production)."
    )
    ...
    for wrap_name, shape, fused_fn in WRAPPERS:
        for M in M_LIST:
            ...
            for bsv_label, bsv_val in BSV_REGIMES:
                ...
                legacy_us = _time_us(
                    lambda: _production_legacy_path(
                        x,
                        R_4d,
                        W,
                        output_sizes,
    ...
    for M in M_LIST:
        ...
        for bsv_label, bsv_val in BSV_REGIMES:
            ...
            input2_max_abs = (
                (input2_out.float() - legacy_out.float()).abs().max().item()
            )
            ...
            input2_us = _time_us(
                lambda: _gate_up_two_output_input_path(
                    x,
                    R_4d,
                    W,
                    output_sizes,
            ...
            full_fused_us = _time_us(
                lambda: fused_rotate_project_gate_up(
                    x,
                    R_4d,
                    W,
                    output_sizes,
    ...
    if all_pass:
        print(
            "All shapes within bf16 tolerance (max_abs <= 2e-3) vs production legacy."
        )
    else:
