...
class OFTRef:
    """Reference record for an OFT adapter. Holds the unified adapter
    identity; ``OFTRegistry`` accesses adapters only through these members.
...
class OFTRegistry:
    ...
    def __init__(self, oft_paths: Optional[List[OFTRef]] = None):
        ...
        # An ordered dictionary to hold OFTRef objects, mapping from adapter name to OFTRef.
        # The OFTRefs are stored in LRU order, such that adapters that have been
        ...
        if oft_paths:
            for ref in oft_paths:
                self._register_oft(ref)

    async def register(self, ref: OFTRef):
        """
        Register a new OFTRef object in the registry.

        Args:
            ref (OFTRef): The OFTRef object to register.
        """
    ...
    async def unregister(self, name: str) -> str:
        """
        Unregister an OFTRef object from the registry and returns the removed adapter ID.
    ...
    async def refresh(self, ref: OFTRef):
    ...
    async def _increment_ref_counters(
        self, refs: List[Optional[OFTRef]]
    ) -> None:
    ...
    async def _acquire_refs(
        ...
        """Atomically snapshot the matching OFTRef(s) and start tracking usage.
    ...
    async def get_unregistered_ofts(self, name: set[str]):
        """
    ...
    def _register_oft(self, ref: OFTRef):
        """
    ...
    def get_all_ofts(self) -> Dict[str, OFTRef]:
        """
        Returns a dictionary of all registered adapters.

        Returns:
            Dict[str, OFTRef]: A dictionary mapping adapter names to OFTRef objects.
        """
