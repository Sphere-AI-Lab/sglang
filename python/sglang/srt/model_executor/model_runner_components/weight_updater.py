...
class WeightUpdater:
    ...
    def discard_adapter_stage(
        self, *, load_format, adapter_name, adapter_id, adapter_version
    ):
        """Drop an exact hidden stage without receiving weights or unloading."""
        try:
            runner = self.get_model_runner()
            if load_format == "lora_adapter" and runner.server_args.enable_lora_staging:
                manager = runner.lora_manager
            elif (
                load_format == "oft_adapter" and runner.server_args.peft_method == "oft"
            ):
                manager = runner.oft_manager
            else:
                return False, "Stage discard requires native LoRA or canonical OFT"
            result = manager.discard_adapter_stage(
                adapter_name, adapter_version, adapter_id=adapter_id
            )
            return result.success, (
                result.error_message
                if not result.success
                else "Adapter stage discarded"
            )
        except Exception as error:
            return False, f"Failed to discard adapter stage: {error}"
